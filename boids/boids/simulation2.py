...
class BoidSimulation2:
    ...
    def __init_boids(self):
        for _ in range(self.__count):
            self.__boids.append(
                Boid(
                    random.random() * self.__width,
                    random.random() * self.__height,
                    random.random() * 10 - 5,
                    random.random() * 10 - 5,
                )
            )
    ...
    def __n_closest_boids(self, boid, n):
        ...
        # sorted.sort(
        #     key=lambda a,b: self.__distance(boid, a) - self.__distance(boid, b)
        # )
    ...
    def display(self):
        ...
        for idx, boid in enumerate(self.__boids):
            ...
            if self.__debug:
                self.__screen.addstr(line, 0, f"({row},{col}) -> [{angle}]/[{boid.dx},{boid.dy}]", curses.color_pair(self.COLOR_BOID))
                line += 1
            else:
